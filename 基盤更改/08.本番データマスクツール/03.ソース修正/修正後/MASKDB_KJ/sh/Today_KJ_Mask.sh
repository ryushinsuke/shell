...
source ~/.bash_profile
# ========================================================================
# システムＩＤ  ：  Today_KJ_Mask
# システム名称  ：  基準系当日分データマスク処理
# 処理概要      ：  基準系当日分データマスク処理
# リターンコード： 0：正常終了
#                  1：異常終了
# 改訂履歴
...
#------------------------------------------------------------------------
#--- 引数設定
DATA_KBN=$1       # CMNと新共通:0  朝一業務と業務共通:1  締め後業務と業務共通:2
MASK_DATE=$2      # 当日日付:1  指定日付:XXXXXXXX
DB_FILE=$3        # 当日DATA用：1  ユーザ指定:/XXXXX/XXXX/XXXX
...
#--- ログファイル設定
LOG_FILE="${LOG_PATH}/Today_KJ_Mask_${DATA_KBN}.log"

#--- CONFファイルを引き込み
CONF_FILE="${CONF_PATH}/Today_KJ_Mask.conf"
if [ -e "${CONF_FILE}" ] ;then
  . "${CONF_FILE}"
else
...
if [ $# -ne 3 ] ;then
  ...
  echo 'usage $3  [ 1 ] 当日用WKDBファイル '                          | tee -a "${LOG_FILE}"
...
#--- 当日用WORK DBファイル指定
if [ "${DB_FILE}" = "1" ];then
   DB_FILE="${CONF_PATH}/${TODAY_WKDB_FILE}"
fi
...
#--- 締め後当日用マスク日付ファイル
SIMEGO_DATE_FILE="${TMP_PATH}/simego_date.tmp"
#--- マスク日付取得
if [ "${MASK_DATE}" = "1" ] ;then
  if [ "${DATA_KBN}" != "2" ];then
    MASK_DATE=`date '+%Y%m%d'`
  else
    if [ -e "${SIMEGO_DATE_FILE}" ] ;then 
      MASK_DATE=`cat "${SIMEGO_DATE_FILE}" | awk '{print $9;}'`
    else
      echo "締め後当日用日付FILE(${SIMEGO_DATE_FILE})がないので、ERR" | tee -a "${LOG_FILE}"
      exit 1
    fi
  fi
fi
...
######################################################################
echo "★-----------------------------------------------------------★"| tee -a "${LOG_FILE}"
echo "◆ ↓当日データマスク処理START(`date`)"                         | tee -a "${LOG_FILE}"
echo "★-----------------------------------------------------------★"| tee -a "${LOG_FILE}"
...
"0")
  ...
  ;;
"1")
  ITK_MASK "${MASK_DATE}" "${ASAITI_WKDB_CONN}" "1"
  ...
  #--- MASKシェル起動
  sh ${SHL_PATH}/${SON_SHELL} "${ASAITI_WKDB_CONN}" "${MASK_DATE}" "1001" "1" &

  #--- NAM 業務共通
  #------ NAM会社、データ区分、DB接続子が占用されるかどうかチェック
  PROCESS_CHECK "${ASAITI_WKDB_10001_CONN}" "10001" "1"
  #--- MASKシェル起動
  sh ${SHL_PATH}/${SON_SHELL} ${ASAITI_WKDB_10001_CONN} ${MASK_DATE} "10001" "1" &
  ...
  #--- MASKシェル起動
  sh ${SHL_PATH}/${SON_SHELL} "${ASAITI_WKDB_CONN}" "${MASK_DATE}" "2001" "1" &

  #--- JTK 業務共通
  #------ JTK会社、データ区分、DB接続子が占用されるかどうかチェック
  PROCESS_CHECK "${ASAITI_WKDB_2000_CONN}" "2000" "1"
  #--- MASKシェル起動
  sh ${SHL_PATH}/${SON_SHELL} ${ASAITI_WKDB_2000_CONN} ${MASK_DATE} "2000" "1" &
  
  #--- ITK 業務共通
  #------ ITK会社、データ区分、DB接続子が占用されるかどうかチェック
  PROCESS_CHECK "${ASAITI_WKDB_1000_CONN}" "1000" "1"
  #--- MASKシェル起動
  sh ${SHL_PATH}/${SON_SHELL} ${ASAITI_WKDB_1000_CONN} ${MASK_DATE} "1000" "1" &
  ;;
"2")
  ITK_MASK "${MASK_DATE}" "${SIMEGO_WKDB_CONN}" "2"
  ...
  #--- MASKシェル起動
  sh ${SHL_PATH}/${SON_SHELL} "${SIMEGO_WKDB_CONN}" "${MASK_DATE}" "1001" "2" &

  #--- NAM 業務共通
  #------ NAM会社、データ区分、DB接続子が占用されるかどうかチェック
  PROCESS_CHECK "${SIMEGO_WKDB_10001_CONN}" "10001" "2"
  #--- MASKシェル起動
  sh ${SHL_PATH}/${SON_SHELL} ${SIMEGO_WKDB_10001_CONN} ${MASK_DATE} "10001" "2" &
  ...
  #--- MASKシェル起動
  sh ${SHL_PATH}/${SON_SHELL} "${SIMEGO_WKDB_CONN}" "${MASK_DATE}" "2001" "2" &

  #--- JTK 業務共通
  #------ JTK会社、データ区分、DB接続子が占用されるかどうかチェック
  PROCESS_CHECK "${SIMEGO_WKDB_2000_CONN}" "2000" "2"
  #--- MASKシェル起動
  sh ${SHL_PATH}/${SON_SHELL} ${SIMEGO_WKDB_2000_CONN} ${MASK_DATE} "2000" "2" &
  
  #--- ITK 業務共通
  #------ ITK会社、データ区分、DB接続子が占用されるかどうかチェック
  PROCESS_CHECK "${SIMEGO_WKDB_1000_CONN}" "1000" "2"
  #--- MASKシェル起動
  sh ${SHL_PATH}/${SON_SHELL} ${SIMEGO_WKDB_1000_CONN} ${MASK_DATE} "1000" "2" &

  #--- 締め後当日用の日付ファイルMV
  mv -f "${SIMEGO_DATE_FILE}" "${SIMEGO_DATE_FILE}_bak"
  ;;
*)
...
#--- 正常終了
echo "★-----------------------------------------------------------★"| tee -a "${LOG_FILE}"
echo "◆ ↑当日データマスク処理 END (`date`)"                         | tee -a "${LOG_FILE}"
echo "★-----------------------------------------------------------★"| tee -a "${LOG_FILE}"
exit 0
