#!/bin/sh
# ========================================================================
# シェルＩＤ    ：  exp_YDC_NAM_SINKYOTU.sh
# シェル名称    ：  基準系本番新共通NAMデータエクスポート処理
# 処理概要      ：  基準系本番新共通NAMデータエクスポート処理
# 入力引数      ：  なし
# リターンコード：  0：正常終了
#                   1：異常終了
# 改訂履歴
...
#本番DB接続子(新共通NAM)のチェック
if [ -z ${NAM_SINKYOTU_CONN} ]
then
  echo  "本番DB接続子(新共通NAM)取得にエラーが発生しました！" | tee -a ${LOG_FILE}
  exit 1
fi
...
#新共通NAMデータエクスポート
echo "基準系本番新共通NAMデータエクスポート処理開始(`date`)" | tee -a ${LOG_FILE}
...
#正常終了
echo "基準系本番新共通NAMデータエクスポート処理終了(`date`)" | tee -a ${LOG_FILE}
exit 0
#基盤更改の修正　END
