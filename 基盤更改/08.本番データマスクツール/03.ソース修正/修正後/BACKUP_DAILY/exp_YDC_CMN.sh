#!/bin/sh
# ========================================================================
# シェルＩＤ    ：  exp_YDC_CMN.sh
# シェル名称    ：  基準系本番共通データエクスポート処理
# 処理概要      ：  基準系本番共通データエクスポート処理
# 入力引数      ：  なし
# リターンコード：  0：正常終了
#                   1：異常終了
# 改訂履歴
...
if [ -z ${CMN_CONN} ]
then
  echo  "本番DB接続子(共通)取得にエラーが発生しました！" | tee -a ${LOG_FILE}
  exit 1
fi
...
#共通データエクスポート
echo "基準系本番共通データエクスポート処理開始(`date`)" | tee -a ${LOG_FILE}
...
#正常終了
echo "基準系本番共通データエクスポート処理終了(`date`)" | tee -a ${LOG_FILE}
exit 0
#基盤更改の修正　END
