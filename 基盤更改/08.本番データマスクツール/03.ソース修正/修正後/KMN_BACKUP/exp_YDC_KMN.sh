...
sh BCZC0070.sh exp "`basename $0 | sed 's/.sh//g'`" ${ORACLE_CNN} ${DB_FILE}/${MASK_KBN}.exp ${MASK_KBN}.log "TABLES=${EXP_TBS}" > ${DB_FILE}/${MASK_KBN}.log 2>&1
#基盤更改の修正　END
#if [ "$?" != 0 ];then
#exit 9
#else
...
#rm -rf ${HOME_PATH}/*.txt
#rm -rf ${HOME_PATH}/*.sql
rm -rf ${HOME_PATH}/log/*.txt
rm -rf ${HOME_PATH}/log/*.sql
rm -rf ${HOME_PATH}/log/*.lst
rm -rf ${DB_FILE}/*.lst
#基盤更改の修正　END
#*********************************************************************
#            正常終了
